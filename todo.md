...
- [ ] **Conflict Resolution**: Implement logic to handle data conflicts if a user edits on multiple devices while offline (last-write-wins vs merge).
- [ ] **Storage Quota Management**: Add alerts when IndexedDB storage usage approaches browser limits.

## 2. Cloud Sync & Collaboration (Expanded)
- [ ] **Multi-Provider Support**:
    - [ ] **Dropbox Integration**: Add OAuth flow and file operations for Dropbox.
    - [ ] **OneDrive Integration**: Add OAuth flow and file operations for Microsoft OneDrive.
    - [ ] **Provider Selector**: UI to allow users to switch between Google Drive, Dropbox, and OneDrive as their primary backup target.
- [ ] **Advanced Sync Logic**:
    ...
    - [ ] **Manual Conflict Resolution UI**: A visual diff tool to let users choose between local and remote versions when a conflict occurs.
- [ ] **Versioning & History**:
...
- [ ] **Inline Autocomplete**: Ghost text suggestions as you type (like GitHub Copilot for prose).
...
- [ ] **Lore Consistency Check**: Cross-reference new text against the Knowledge Base in real-time.
- [ ] **Custom AI Personas**: Allow users to define different "AI Editor Personas" (e.g., "Ruthless Editor", "Encouraging Coach", "Grammar Nazi").

## 5. Knowledge Base & World Building
- [ ] **Wiki-style Linking**: Auto-link names and terms in the text to their Knowledge Base entries.
- [ ] **Visual Relationship Map**: Interactive node graph showing relationships between characters (Family trees, alliances).
- [ ] **Timeline View**: Visual timeline of events extracted from the Knowledge Base and Chapter summaries.
- [ ] **Map Builder**: (Future) Simple tool to generate or upload world maps and pin locations.

## 6. Audio & Accessibility
- [ ] **Voice Control**: Allow users to dictate text (Speech-to-Text) directly into the editor.
...
- [ ] **Pull-to-Refresh**: Implement pull-to-refresh for syncing status (if applicable).
- [ ] **Install Prompts**: Custom UI to encourage users to "Add to Home Screen".
- [ ] **Share Target API**: Allow users to "Share" text/links from other apps *into* AI Book Studio.
