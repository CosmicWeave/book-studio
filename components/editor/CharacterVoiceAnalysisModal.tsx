...
import { CharacterVoiceInconsistency } from '../../types';
...
interface CharacterVoiceAnalysisModalProps {
    isOpen: boolean;
    onClose: () => void;
    results: CharacterVoiceInconsistency[] | null;
    isLoading: boolean;
    onApplySuggestion: (original: string, replacement: string) => void;
}

const CharacterVoiceAnalysisModal: React.FC<CharacterVoiceAnalysisModalProps> = ({ isOpen, onClose, results, isLoading, onApplySuggestion }) => {
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 w-full max-w-4xl max-h-[90vh] flex flex-col border border-gray-200 dark:border-gray-700" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                        <Icon name="USER" className="w-6 h-6 text-indigo-500" />
                ...
                <div className="overflow-y-auto pr-2 -mr-2 flex-grow">
                    {isLoading ? (
                         <div className="flex flex-col items-center justify-center p-8 min-h-[300px]">
                            ...
                            <p className="mt-4 font-semibold text-gray-700 dark:text-gray-300">Analyzing dialogue consistency...</p>
                        </div>
                            ...
                            {results.map((item, index) => (
                                <div key={index} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                                    <div className="bg-zinc-50 dark:bg-zinc-700/30 p-3 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
                                        <span className="font-bold text-indigo-600 dark:text-indigo-400">{item.characterName}</span>
                                        <span className="text-xs text-red-500 font-medium">Inconsistency Detected</span>
                                    </div>
                                    <div className="p-4 space-y-3">
                                        <div>
                                            <p className="text-xs font-bold text-zinc-500 uppercase mb-1">Problematic Dialogue</p>
                                        ...
                                        <div>
                                            <p className="text-xs font-bold text-zinc-500 uppercase mb-1">Reason</p>
                                            <p className="text-sm text-zinc-600 dark:text-zinc-400">
                                                {item.inconsistencyReason}
                                            </p>
                                        </div>
                                        <div>
                                            ...
                                            <div className="flex items-start gap-2">
                                                ...
                                                <button 
                                                    onClick={() => onApplySuggestion(item.dialogue, item.suggestedFix)}
                        ...
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-bold text-zinc-800 dark:text-zinc-100">Excellent!</h3>
                            <p className="text-zinc-500 dark:text-zinc-400 mt-2 max-w-md mx-auto">
                                The AI didn't find any significant inconsistencies in character voice for this chapter.
                            </p>
                        </div>
                    )}
                </div>
